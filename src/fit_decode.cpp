...
// CRC-16 lookup table and inline helper — identical to CRC::Get16 in fit_crc.cpp.
// Defined here so the compiler can inline and unroll it in the batch fast path without
// requiring link-time optimisation. Do NOT call this from outside fit_decode.cpp;
// use CRC::Get16 / CRC::Calc16 for all other callers.
namespace {
    static const FIT_UINT16 kCrcTable[16] = {
        0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00, 0x2800, 0xE401,
        0xA001, 0x6C00, 0x7800, 0xB401, 0x5000, 0x9C01, 0x8801, 0x4400
    };
    inline FIT_UINT16 UpdateCRC(FIT_UINT16 crc, FIT_UINT8 byte)
    {
        FIT_UINT16 tmp;
        tmp = kCrcTable[crc & 0xF];
        crc  = (crc >> 4) & 0x0FFF;
        crc  = crc ^ tmp ^ kCrcTable[byte & 0xF];
        tmp = kCrcTable[crc & 0xF];
        crc  = (crc >> 4) & 0x0FFF;
        crc  = crc ^ tmp ^ kCrcTable[(byte >> 4) & 0xF];
        return crc;
    }
}
                ...
                if (canCopy > 0)
                {
                    if (skipHeader == FIT_FALSE)
                    {
                        for (FIT_UINT32 i = 0; i < canCopy; i++)
                            crc = UpdateCRC(crc, (FIT_UINT8)buffer[currentByteIndex + i]);
...
void Decode::ExpandComponents(Field *containingField, const Profile::FIELD_COMPONENT *components, FIT_UINT16 numComponents)
{
    ...
    for (i = 0; i < numComponents; i++)
    {
        ...
        if (component->num != FIT_FIELD_NUM_INVALID)
        {
            ...
            FIT_FLOAT64 value;
            FIT_UINT64 bitsValue = FIT_UINT64_INVALID;
            FIT_SINT64 signedBitsValue = FIT_SINT64_INVALID;
            ...
            {
                signedBitsValue = containingField->GetBitsSignedValue64(offset, component->bits);

                if (signedBitsValue == FIT_SINT64_INVALID)
            ...
            {
                bitsValue = containingField->GetBitsValue64(offset, component->bits);

                if (bitsValue == FIT_UINT64_INVALID)
                    break; // No more data for components.
            ...
            else if (componentField.GetNumComponents() > 1)
            {
                ...
                {
                    mask = ((FIT_UINT64)1 << baseTypeSizes[componentField.GetType() & FIT_BASE_TYPE_NUM_MASK]) - 1;
                    if (mesg.HasField(componentField.GetNum()))
                    {
